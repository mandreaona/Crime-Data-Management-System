...
SELECT year(c.crime_datetime) as "Year", COUNT(c.crime_id) as "Crimes in thE US", ct.violent as "Violent Crimes"
...
SELECT cy.name as "City", COUNT(*) as "Number of Crimes", cy.state
...
select cy.name as "City", c.crime as "Crime", COUNT(crime_id) as "Number of Crimes" FROM crime c, address a, city cy
WHERE c.address_id = a.address_id
AND a.city_id = cy.city_id
GROUP BY c.crime, cy.name ORDER BY city;
...
FROM n_crimes_per_month, city cy
GROUP BY Months
...
SELECT cy.name as "City", cy.state as "State", a.neighborhood as "Neighborhood", COUNT(crime_id) as "Number of Crimes" 
FROM crime c, address a, city cy
WHERE c.address_id = a.address_id
AND a.city_id = cy.city_id
GROUP BY a.neighborhood, cy.name, cy.state
ORDER BY cy.name, a.neighborhood;
...
SELECT cy.name as "City", a.neighborhood, COUNT(p.person_id) as "Number of Offenders", cy.state as "State"
FROM offender o, person p, crime c, address a, person_address pa, city cy
WHERE c.crime_id = o.crime_id
AND o.person_id = p.person_id
AND p.person_id = pa.person_id AND pa.type = "Home"
AND pa.address_id = a.address_id AND cy.city_id = a.city_id
GROUP BY ct.name, a.neighborhood, cy.state;
...
FROM victim v, offender o, crime cr, person po, person pv, address a, city ct WHERE cr.crime_id = o.crime_id
...
SELECT c.crime as "Crime", p.race as "Race/Ethnicity", COUNT(o.crime_id) as "Number of offenders" FROM person p, crime c, offender o, crime_type ct
WHERE p.person_id = o.person_id
AND o.crime_id = c.crime_id
AND ct.crime = c.crime AND c.crime = "Murder" GROUP BY p.race, c.crime;
...
SELECT cy.name as "City",TRUNCATE(avg(p.age), 0) as "Average age" FROM crime c, address a, city cy, crime_type ct, offender o, person p WHERE c.address_id = a.address_id
AND c.crime = ct.crime
AND a.city_id = cy.city_id
